 ...
 *
 * input  = utf8(LABEL_SESSION_ID) || introducerPub || alicePub || bobPub
 * result = SHA-256(input)[0..16] encoded as base64url
 *
 * All pubkeys must be 32-byte Ed25519 pubkeys (raw bytes, not base64url).
 * Callers holding b64u strings decode first: `fromB64u(b64uStr)`.
 *
 * @param introducerPub - Introducer long-term Ed25519 pubkey (32 bytes)
 * @param alicePub      - Alice ephemeral pubkey (32 bytes)
 * @param bobPub        - Bob ephemeral pubkey (32 bytes)
 */
export function deriveSessionId(
  ...
  const input  = concatBytes(utf8(LABEL_SESSION_ID), introducerPub, alicePub, bobPub);
  const digest = sha256(input);
  return b64uEncode(digest.slice(0, 16));
}
 ...
 *
 * ikm  = X25519(ownEphPriv, peerEphPub)
...
/**
 * Computes HMAC-SHA256(ownMacKey, utf8(LABEL_AUTH_MAC) || transcript).
 */
export function computeAuthMac(ownMacKey: Uint8Array, transcript: Uint8Array): Uint8Array {
  return hmac(sha256, ownMacKey, concatBytes(utf8(LABEL_AUTH_MAC), transcript));
}

/**
 * Verifies the peer's auth MAC.
 *
 * Recomputes HMAC-SHA256(peerMacKey, utf8(LABEL_AUTH_MAC) || canonicalTranscript)
 * and compares using constant-time equality.
 *
...
export function verifyAuthMac(
  ...
  const expected = hmac(sha256, peerMacKey, concatBytes(utf8(LABEL_AUTH_MAC), canonicalTranscript));
 ...
 *
 * nonce = HMAC-SHA256(ownMacKey, utf8(LABEL_AUTH_NONCE) || transcript)
...
export function computeAuthSig(
  ...
  const nonce   = hmac(sha256, ownMacKey, concatBytes(utf8(LABEL_AUTH_NONCE), transcript));
 ...
 *
 * nonce = HMAC-SHA256(peerMacKey, utf8(LABEL_AUTH_NONCE) || canonicalTranscript)
...
export function verifyAuthSig(
  ...
  try {
    const nonce   = hmac(sha256, peerMacKey, concatBytes(utf8(LABEL_AUTH_NONCE), canonicalTranscript));
    const message = concatBytes(utf8(LABEL_AUTH_SIGN), nonce);
...
type AeadLabel = typeof LABEL_AUTH_MAC | typeof LABEL_ACTIVATE_MAC;
